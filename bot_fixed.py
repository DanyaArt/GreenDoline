...
def process_login_phone(message):
    ...
    if user:
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.add(types.KeyboardButton('Отмена'))
        msg = bot.send_message(message.chat.id, "Пользователь не найден. Проверьте номер телефона или зарегистрируйтесь.", reply_markup=markup)
        bot.register_next_step_handler(msg, process_login_phone)
...
def process_login_password_by_phone(message, user):
    ...
    if user[8] == password_hash:  # password_hash находится в индексе 8
        with sqlite3.connect('users.db', check_same_thread=False) as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE users SET user_id=? WHERE phone=?", (message.from_user.id, user[4]))
            conn.commit()
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.add(types.KeyboardButton('Отмена'))
        msg = bot.send_message(message.chat.id, "Неверный пароль. Попробуйте снова:", reply_markup=markup)
        bot.register_next_step_handler(msg, lambda m: process_login_password_by_phone(m, user))

def start_registration(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(types.KeyboardButton('Отмена'))
    msg = bot.send_message(message.chat.id, "Введите вашу фамилию:", reply_markup=markup)
...
def process_last_name(message):
    ...
    if not last_name or len(last_name) < 2:
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.add(types.KeyboardButton('Отмена'))
        msg = bot.send_message(message.chat.id, "Фамилия должна содержать минимум 2 символа. Попробуйте снова:", reply_markup=markup)
        return bot.register_next_step_handler(msg, process_last_name)
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(types.KeyboardButton('Отмена'))
...
def process_first_name(message, last_name):
    ...
    if not first_name or len(first_name) < 2:
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.add(types.KeyboardButton('Отмена'))
        msg = bot.send_message(message.chat.id, "Имя должно содержать минимум 2 символа. Попробуйте снова:", reply_markup=markup)
        return bot.register_next_step_handler(msg, lambda m: process_first_name(m, last_name))
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(types.KeyboardButton('Отмена'))
...
def process_middle_name(message, last_name, first_name):
    ...
    if not middle_name or len(middle_name) < 2:
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.add(types.KeyboardButton('Отмена'))
        msg = bot.send_message(message.chat.id, "Отчество должно содержать минимум 2 символа. Попробуйте снова:", reply_markup=markup)
        return bot.register_next_step_handler(msg, lambda m: process_middle_name(m, last_name, first_name))
    markup = types.ReplyKeyboardMarkup(one_time_keyboard=True)
    reg_button = types.KeyboardButton(text="📱 Отправить номер", request_contact=True)
    markup.add(reg_button)
    markup.add(types.KeyboardButton('Отмена'))
    msg = bot.send_message(message.chat.id, "Отправьте ваш номер телефона:", reply_markup=markup)
...
def show_profile(message):
    ...
    if user:
        ...
        register_date = user[10]  # register_date находится в индексе 10
        ...
        # Обработка школы
        school = user[6]  # school находится в индексе 6
